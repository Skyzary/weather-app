...
export const weatherService = {
  ...
  async getGeo(city: string): Promise<CityCoords[]> {
    ...
    try {
      if (city) {
        ...
        if (response.data && response.data.length > 0) {
          return response.data.map((item: CityCoords) => ({
            lat: item.lat,
            lon: item.lon,
            name: item.name,
      ...
      if (axios.isAxiosError(error) && error.message) {
        ...
        if (error.response?.status !== 401) {
          throw new Error(`Error fetching geo data: ${error.message} `, { cause: error });
        }
      }
      throw error;
    }
  ...
  async fetchWeather(coords: CityCoords, language: string = 'en'): Promise<CurrentWeatherData | undefined> {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.message) {
        ...
        if (error.response?.status !== 401) {
          throw new Error(`Error fetching weather data: ${error.message} `, { cause: error });
        }
      }
      throw error;
    }
  ...
  async getForecast(coords: CityCoords | undefined, language: string = 'en'): Promise<ForecastData | undefined> {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.message) {
        ...
        console.log('Error is' + error.message, error.cause);
        if (error.response?.status !== 401) {
          throw new Error(`Error forecasting data: ${error.message} `, { cause: error });
        }
      }
      throw error;
    }
